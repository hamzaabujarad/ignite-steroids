...
beforeEach(() => (MMKV.getItem as jest.Mock).mockReturnValue(Promise.resolve(VALUE_STRING)))
...
test("save", async () => {
  ...
  expect(MMKV.setItem).toHaveBeenCalledWith("something", VALUE_STRING)
})
...
test("remove", async () => {
  remove("something")
  expect(MMKV.removeItem).toHaveBeenCalledWith("something")
})

test("clear", async () => {
  clear()
  expect(MMKV.clearStore).toHaveBeenCalledWith()
})
